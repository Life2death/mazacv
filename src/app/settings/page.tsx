...
export default function SettingsPage() {
  ...
  async function handleFindJobs() {
    setError("");
    setJobs([]);
    const skillsStr = settings.keywords.trim();
    if (!skillsStr) {
      return setError("Pehle skills daal — keywords field khaali hai.");
    }
    ...
    const jobTitles = settings.jobTitles
      ? settings.jobTitles.split(/[,;]+/).map((s) => s.trim()).filter(Boolean)
      : undefined;
    ...
    <main className="mx-auto max-w-3xl px-4 py-10">
      ...
      <section className="mb-6 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <div className="mb-4">
          ...
          <p className="mb-1 text-xs text-slate-400">Resume se auto-fill hoga — edit kar sakte ho</p>
          <textarea
        ...
        <div className="mb-4">
          <label className="block text-sm font-medium text-slate-700">Target Job Titles</label>
          <input
            type="text"
            value={settings.jobTitles}
            onChange={(e) => updateSettings({ jobTitles: e.target.value })}
            placeholder="Senior React Developer, Frontend Lead"
